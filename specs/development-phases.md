...
| **D8** | API key handling | **Server-side secrets only.** Environment variables preferred, gitignored `config.yaml` accepted. The server never returns a key, and the browser never stores one. Keys stay out of `localStorage`, out of the bundle, and out of Git. **Narrowed by D34 for 1.0:** the UI may *not* POST a key — `P1-2` took the environment-only reading and D34 makes it binding, so the settings screen reports a key's *presence* and nothing else. | P1, P4 |
...
### Resolved by the writer (Phase 4, 2026-09-04)

Put to the writer in the Phase 4 plan § 2 and accepted **as recommended**, all five, on the
day the plan was written. Binding on the same terms as D1–D29. The plan's § 2 table stays as
the reasoning behind them, including the alternative weighed for each and where it bites if
the answer is wrong. **D34 narrows D8 for 1.0**; D31 is the one place this phase deliberately
builds ahead of its consumer.

| ID | Decision | Binding resolution | Affects |
|---|---|---|---|
| **D30** | Where a chat conversation lives, and whether Phase 4 writes into Phase 6's tables | **Its own two tables in the project file — `conversation` and `message` — and a Phase 4 turn is explicitly *not* a run.** Migration 004. A conversation is project-scoped (D6) and soft-deleted (D22, D25); a message stores the **composed context that produced it** beside the text and the token usage, because that record is the only way a bad answer is ever diagnosed. Phase 6's `run` table, when it arrives, **references a message** rather than replacing it: a run is what produced one assistant turn. Holding the conversation in React state was rejected on D13's own logic — the writer paid for the answer, and a reload must not make them pay twice. Writing Phase 6's `run`/`run_step` shape now was also rejected: it is a schema guessed two phases early around a completion that has no plan and no tool calls, and D20's forward-only migrations make guessing wrong a migration to undo rather than an edit. | P4, P6, P7 |
| **D31** | Whether the port carries tool declarations before any tool exists | **Yes. `CompletionRequest.tools` and `CompletionResult.tool_calls` are in the port from `P4-2`, and Phase 4 declares no tools and calls none.** Both adapters' translation of them and the prompted-JSON fallback (`P4-7`) are built and tested here against `FakeProvider` and **recorded provider fixtures**; no product surface uses them. This is the one place Phase 4 deliberately builds ahead, on D28's precedent: normalising a tool call is the hardest and most provider-divergent part of an adapter, and discovering its shape in Phase 6 means rewriting both adapters, the fake, and the fallback at the moment the agent loop is also new. The counter-argument stays on the record — a shape with no consumer is a shape nobody has proved sufficient — and the mitigation is that the **fixtures are its consumer**: a real Anthropic and a real OpenAI tool-call payload, recorded once, translated in both directions, asserted equal. | P4, P6 |
| **D32** | The streaming envelope, and whether chat and Phase 6's runs share one | **One event vocabulary over one WebSocket, defined in Phase 4 and extended — never replaced — by Phase 6.** D11 chose the transport; this fixes what travels on it. Phase 4 emits `start`, `delta`, `usage`, `done`, and `error`; Phase 6 adds `plan`, `tool_call`, `tool_result`, and `step` to the same union. **The asymmetry is deliberate and is tested in both suites:** the server *rejects* an event type it does not know, and the client *ignores* one — so a server ahead of a stale bundle degrades to less detail rather than to a broken panel. A plain HTTP streaming response for chat now was rejected as the second mechanism D11 exists to avoid, and because it forecloses the mid-stream interaction D11 named as its reason — cancel, which is wanted in this phase. | P4, P6 |
| **D33** | What accepting a proofread, tone, or rewrite result does | **It applies through the editor as an ordinary transaction, and the result record is ephemeral.** The panel renders an explicit before/after (D12); accepting dispatches a normal ProseMirror transaction, so it is undoable, autosaved, and covered by the snapshot machinery that already exists. Nothing is written to a `proposal` table, because there is none until Phase 7. D5's queue is about *agent writes to the bible* — asynchronous, possibly bulk, reviewed later; a rewrite of the paragraph under the cursor is synchronous, single-target, and accept-or-discard, and giving it a queue would produce a list of stale suggestions against passages that have since been rewritten. Each action mints its range's anchor through `AnchorStore` and by no other means, and **never applies to a `stale` one**. | P4, P7 |
| **D34** | How the API key reaches the server, and what the settings UI may do with it | **The key stays environment-only, and the settings UI is read-only about it.** This keeps `P1-2`'s narrowing of D8: `ARCHETYPE_ANTHROPIC_API_KEY` and `ARCHETYPE_OPENAI_API_KEY` are `SecretStr` fields, `GET /api/settings` returns `Settings.public_dump()`, and the UI shows **whether a key is present** and never its value, its length, or its prefix. Non-secret provider settings — provider, base URL, model id, budgets, capability flags — are writable and persist to `config.yaml`. Letting the UI POST a key, which D8's own wording permits, was rejected for 1.0 on the grounds `P1-2` already took: accepting a key means writing it somewhere, and the app writing a secret to a file it chose is a new class of mistake — wrong permissions, a backup that captures it, a `config.yaml` that stops being gitignored. The cost is accepted and stated plainly: changing provider is an environment edit and a restart. If that proves intolerable in use, **D34 is the entry to reopen**, and it reopens as a deliberate act. | P4, P9 |
