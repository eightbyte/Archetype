...
**Status:** **Active — § 2 ruled, Groups A and B complete** · **Version:** 1.2 · **Date:** 2026-08-30
...
### Carried into Group C

- **Nothing in `web/` consumes anchors yet.** `SaveResult` grew its `anchors` field and the client
  types mirror it, so a save's answer is already typed and contract-checked; P2-9 is what draws
  it. The fake API client returns an empty list deliberately — a fake that invented resolutions
  would be a second resolver with no specification behind it.
- **The five anchor routes have no client methods.** `ApiClient` is untouched; P2-10 adds them
  with the surface that calls them.
- **B4 is ruled, not open** (2026-08-30). A paragraph split through an anchored range keeps the
  anchor; only a split with new words written into the gap breaks it. P2-9's decorations must
  therefore survive a range that spans a block boundary, and P2-15 carries the ruling into
  `anchors.md` § 4 alongside § 10.
