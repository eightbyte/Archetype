...
**Status:** **In progress (2026-09-05)** — **§ 2 is ruled**: D30–D34 were put to the writer on
2026-09-04 and accepted **as recommended**, all five; they are promoted to the register and
binding. **Groups A, B, and C are delivered** (2026-09-04, 2026-09-05, 2026-09-05): the port, the
fake, migration 004, both adapters, the prompted-JSON fallback, the registry, and now the
conversation store, the chat routes, the **WebSocket**, the context composer, and the settings
surface. **The application can hold a streamed conversation and persist it** — against
`FakeProvider` in the suite, and against a real provider from a shell. **Group D is not started**:
there is no panel, no *Ask agent*, no diff view, and no settings screen, so nothing a *writer* can
click reaches a model yet. Both suites are green (**1,373** backend, 544 frontend).
...
**Group D** will add a block here as it lands.
