...
- [`specs/providers.md`](providers.md) — the `LLMProvider` port, the normalised request and result, the stream event vocabulary, the capability flags, the error taxonomy, and what a provider may **not** be asked to do (written at `P4-1`, before the code it governs)
...
**Changes in 1.9 (2026-09-05):** **Phase 4 Group B is delivered** — the adapters, the fallback, and the registry (`P4-5` … `P4-8`). Both providers are reachable through one port: `llm/adapters/anthropic.py` over the Messages API and `llm/adapters/openai_compat.py` over the chat-completions shape, with one shared `transport.py` and one SSE parser under them; `prompted_tools.py` is D31's fallback, invisible above the port; `llm/budget.py` is ruling 7's hard refusal; `llm/registry.py` is the only place a provider is constructed. **No architecture changed**: § 3's table already fixed LLM access as *HTTP via `httpx`, one adapter per provider*, and that is what was built — **no vendor SDK is used at all**, so ruling 2's guard now holds in a stronger form and a second import-graph test fences `httpx` into `llm/`. `httpx` moves from a development dependency to a runtime one, which is the only change to the dependency budget and adds nothing that was not already installed. Both suites green — **1,293 backend, 535 frontend**. Ten as-built deviations, in that plan's § 7, and three of them are corrections to `specs/providers.md` now carried in its § 13. **`B1` is the one to read**: the recorded fixtures are **transcribed from each provider's published wire format rather than captured from a live account** — the capture scripts are committed but have not been run, because a capture makes billed requests — so this group's tests prove the adapters match what the documentation says, not yet what the providers actually send. § 8's steps 4 and 12 are where that gap closes, and refreshing the corpus from that run is one command per provider. Groups C and D are not started; nothing in the app calls a model yet.

**Changes in 1.8 (2026-09-04):** **Phase 4's § 2 is ruled.** All five proposed register entries were put to the writer and accepted **as recommended** — D30 a conversation lives in its own two tables and a Phase 4 turn is *not* a Phase 6 run; D31 the port carries tool declarations from `P4-2` with recorded provider fixtures as their only consumer; D32 one stream event vocabulary over one WebSocket, extended by Phase 6 and never replaced, with the server rejecting an unknown event type and the client ignoring one; D33 an accepted rewrite is an ordinary editor transaction and nothing durable until Phase 7; D34 the API key stays environment-only and the settings UI is read-only about it. They are promoted to the register in [`specs/development-phases.md`](development-phases.md) § 1 and binding on the same terms as D1–D29. **D34 narrows D8**, whose own wording permitted the UI to POST a key: `P1-2` took the environment-only reading in code and this makes it a decision rather than an implementation detail. No phase boundary moved, no scope changed, and no earlier decision was reversed. **Phase 4 Group A is delivered** (2026-09-04): `specs/providers.md` written first at `P4-1`; the port and its closed vocabularies at `P4-2`, with the import-graph test that keeps a provider SDK out of everything above `llm/adapters/`; `FakeProvider` at `P4-3`; and migration 004 with its two tables at `P4-4`, proved against a captured version-3 project file. Both suites green — **1,180 backend, 535 frontend**. Six as-built deviations, in that plan's § 7; the one that changes a stored shape is `A1`, a `stop_reason` column on `message` so that a **cancelled** turn still reads as cancelled after a reload.

**Changes in 1.7 (2026-09-04):** **Phase 3 is closed.** Its manual acceptance script (phase-3 plan § 8) was run by hand against the single-process build; **all fifteen steps pass**, and all ten exit criteria are met. Both suites green — 1,147 backend, **526 frontend**. It earned the phase boundary the way Phase 2's did, and in the same place: **step 12 found a real defect no test could have caught.** Rewriting a marked passage *wholesale* — new sentences rather than an edit — leaves the anchor `stale` with its positions deliberately unmoved, and the editor was still drawing that range as a **wavy red underline**, which is the browser's own spellcheck idiom; the writer read it as a spelling squiggle and reported no highlight at all. The range itself was landing on words the anchor never referred to — a mid-word fragment of the replacement. The whole Phase 2 anchor corpus edits a character or a word wherever it touches this path, so nothing on either side of the wire said so. Two answers were put to the writer and **the ruling was to keep the range and restyle it**: the positions stay the server's answer, unedited, and how the mark reads becomes the guard. `.anchor-stale` is now a tinted alarm wash under a solid rule, two tests pin the wholesale-rewrite case, `specs/anchors.md` § 1 carries what a stale range does **not** promise, and step 12 was re-run against the fix. A second, smaller finding: **§ 8's step 3 cannot be performed by hand at all** — an `enum` is a `<select>` of its declared members, so the UI offers no way to submit an illegal value and the server refusal it asked for is unreachable, which is D26's *unbuildable, not refused* working exactly as intended (phase-3 plan § 7, `E1` and `E2`). No phase boundary moved and no decision changed. **Next: Phase 4 — the LLM provider layer and chat**, planned in [`specs/phase-4-plan.md`](phase-4-plan.md); its § 2 proposes **D30–D34** and none is binding until ruled on. One child document is added: `specs/providers.md`, to be written at `P4-1` before the code it governs, on the `P2-4` and `P3-1` pattern.
