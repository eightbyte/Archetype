...
- **No AI, no chat, no WebSocket, no provider, no extraction.** Still not even a stub route
  (api-contract's *deliberately absent* list). The `proposed` and `rejected` entry statuses and the `agent` origin are
...
| **4** | **Phase 3's entry search is a `LIKE` filter on the list route**, `GET /api/projects/{pid}/entries?q=`, over `name`, aliases, and `summary`. It is **not** `/search`, and it does not touch FTS5. | Phase 5 owns search and owns that route name. A Phase 3 `/search` that later means something else is a route a client comes to depend on with the wrong meaning — the reason api-contract refuses stub routes. A bible is hundreds of rows; `LIKE` is correct at that size and is honest about being a filter. |
